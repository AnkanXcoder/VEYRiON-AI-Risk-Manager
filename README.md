...
VEYRiON evaluates a transaction using an XGBoost fraud model, converts the predicted probability into an operational `APPROVE`, `REVIEW`, or `BLOCK` decision, and exposes local SHAP evidence for model transparency.
...
The free Render instance may spin down after inactivity, so the first request can take longer to respond.
